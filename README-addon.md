...
- `REV_PitchAlign` — `1` (default) WSOLA-style correlation-aligned tap jumps: removes the comb/"doubled voice" coloration on voiced speech in exchange for ~12 ms extra latency and slight micro-timing jitter at grain boundaries. `0` = classic dual-tap shifter (kept for A/B). Takes effect from the speaker's next utterance.
